...
initOpenTelemetry('orders-service');
...
async function bootstrap() {
  ...
  const app = await NestFactory.create(OrdersServiceModule, { logger });
  ...
  logger.log(`🚀 Orders Microservice running on TCP & HTTP metrics on ${host}:${port}`);
}
bootstrap();
